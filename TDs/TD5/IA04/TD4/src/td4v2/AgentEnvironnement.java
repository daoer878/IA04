...
public class AgentEnvironnement extends Agent {
	...
	private Cell grille[][];
	private SendRequest requestSender;
	private String fichierPath ="src/sudoku.txt";
	...
	private void readFile() {
		...
		//cr闁峵ion ou ajout dans le fichier texte
		/*
		try {
			FileWriter fw = new FileWriter (fichier);
			BufferedWriter bw = new BufferedWriter (fw);
			PrintWriter fichierSortie = new PrintWriter (bw); 
				fichierSortie.println (chaine+"\n test de lecture et 闁弐iture !!"); 
			fichierSortie.close();
			System.out.println("Le fichier " + fichier + " a 闁狅拷cr娈�"); 
		}
		catch (Exception e){
			System.out.println(e.toString());
		}	
		*/
		requestSender = new SendRequest();
	...
	public void printNewGrid() {
		for(int i = 0; i < 9; i++){
			for(int j = 0; j < 9; j++) {
				if(grille[i][j].getValue() == 0 && grille[i][j].getPossibles().size() == 0)
					System.out.print("E");
				else
					System.out.print(String.valueOf(grille[i][j].getValue()));
			}
	...
	public void allowNextIteration() {
		this.requestSender.resetZoneToAllocate();
	}
	...
	public class SendRequest extends CyclicBehaviour {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 2292596489563164741L;
		private ArrayList<Integer> zoneToAllocate;
		
		void resetZoneToAllocate() {
			zoneToAllocate.clear();
	...
	public class RecieveAnalysLisener extends CyclicBehaviour{
		...
		private ArrayList<Integer> zoneDone;
		
		public RecieveAnalysLisener() {
			zoneDone = new ArrayList<Integer>();
			zoneDone.clear();
		}
		...
		public void action() {
			...
			if(message != null)
			{
				...
				try {
					...
					zoneDone.add(data.getZone());
					addBehaviour(new RenewTable(data, message));
					
					if(zoneDone.size() == 27) {
						zoneDone.clear();
						((AgentEnvironnement)myAgent).allowNextIteration();
						((AgentEnvironnement)myAgent).printNewGrid();//DEBUG ---------------
						...
						{
							System.out.println("\nSudoku found out !\n");
							ACLMessage endMessage = new ACLMessage(ACLMessage.CANCEL);
							endMessage.setContent("Stop");
							endMessage.addReceiver(new AID("Simulation", AID.ISLOCALNAME));
	...
	public class RenewTable extends Behaviour {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 5608632084372816126L;
		...
		public void action() {
			...
			if (zoon>=0 && zoon <9){
				...
				for(int i = 0; i < 9; i++) {
					...
					//reset value of cellul
					if(grille[zoon][i].getValue() == 0 && table[i].getValue() != 0)
					{
						grille[zoon][i].setValue(table[i].getValue());
						grille[zoon][i].getPossibles().clear();
					}
				}
			...
			//column
				...
				for(int i = 0; i < 9; i++) {
					...
					if(grille[i][zoon].getValue() == 0 && table[i].getValue() != 0)
					{
						grille[i][zoon].setValue(table[i].getValue());
						grille[i][zoon].getPossibles().clear();
					}
				}
			...
			//square
				...
				for(int i=0; i < 3; i++) {
					for(int j = 0; j < 3; j++) {
						...
						if(grille[l][c].getValue() == 0 && table[(i*3) + j].getValue() != 0)
						{
							grille[l][c].setValue(table[(i*3) + j].getValue());
							grille[l][c].getPossibles().clear();
						}
					}
				}
